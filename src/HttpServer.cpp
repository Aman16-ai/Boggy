...
// Current implementation we are creating two router object object which wrong that is why we are not able to access the router in execute function.
...
void HttpServer::handle_client(int client_socket)
{
    ...
    // Router router;
    this->router.execute(req.getMethod(), req.getpath(), req, res);
    if (!(req.getMethod() == "GET"))
    {
        ...
        res.send_response("Hi from the backend ", "text/html");
    }
